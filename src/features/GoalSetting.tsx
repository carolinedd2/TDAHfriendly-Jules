...
const QuickGoalButton: FC<{ label: string; onClick: () => void }> = ({
  label,
  onClick,
}) => (
  <button
  ...
  return (
    <div className="mt-3 space-y-4">
      <div className="flex flex-col sm:flex-row items-center sm:space-x-3 space-y-3 sm:space-y-0">
        <label htmlFor="subtopic-goal-input" className="sr-only">
          Número de subtópicos para estudar
        </label>
        <input
          ...
          value={subtopicGoal}
          onChange={e =>
            setSubtopicGoal(Math.max(1, parseInt(e.target.value, 10) || 1))
          }
          min="1"
      ...
      <div className="flex flex-col sm:flex-row items-center sm:space-x-3 space-y-3 sm:space-y-0">
        <label htmlFor="pomodoro-goal-input" className="sr-only">
          Número de Pomodoros para completar
        </label>
        <input
          ...
          value={pomodoroGoal}
          onChange={e =>
            setPomodoroGoal(Math.max(1, parseInt(e.target.value, 10) || 1))
          }
          min="1"
          className="w-full sm:w-20 px-3 py-2 text-center border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
