...
import { TRAIL_DETAILS, ORIGINAL_COLOR_TO_TAILWIND_MAP } from '../constants';
...
export const ReviewSection: FC<ReviewSectionProps> = ({ reviewSchedule, onOpenReviewModal, allTopicsMeta }) => {
  const topicsDueForReview = allTopicsMeta.filter(
    meta => reviewSchedule[meta.id]?.isDueForReview
  );

  return (
    <section className="mb-10 p-6 bg-white rounded-2xl shadow-lg" aria-labelledby="review-section-title">
      <h3 id="review-section-title" className="text-xl font-semibold text-gray-800 mb-1">Revisão Programada 🧠</h3>
      <p className="text-sm text-gray-500 mb-5">Tópicos que precisam da sua atenção para fixar o conhecimento.</p>
      ...
      {topicsDueForReview.length === 0 ? (
        <div className="text-center py-6 px-4 bg-green-50 rounded-lg border-2 border-dashed border-green-200">
          ...
          <h4 className="mt-2 text-md font-medium text-green-700">Tudo em Dia!</h4>
          <p className="mt-1 text-sm text-green-600">Nenhum tópico para revisar hoje. Bom trabalho!</p>
        </div>
      ) : (
        <div className="space-y-3">
          {topicsDueForReview.map(topicMeta => {
            const trailInfo = topicMeta.trail ? TRAIL_DETAILS[topicMeta.trail] : null;
            const topicColor = topicMeta.color ? ORIGINAL_COLOR_TO_TAILWIND_MAP[topicMeta.color] : 'bg-gray-500';
            ...
            return (
              ...
              >
                <div>
                  <h4 className="text-md font-medium text-gray-800">{topicMeta.title}</h4>
                  {trailInfo && (
                   ...
                   {reviewSchedule[topicMeta.id] && (
                    <p className="text-xs text-red-600 font-semibold mt-0.5">
                        Revisão {reviewSchedule[topicMeta.id].reviewStage + 1} de {reviewSchedule[topicMeta.id].reviewStage < 4 ? 5 : 'Final'}
                    </p>
                  )}
                </div>
                <button
